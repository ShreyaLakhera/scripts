...
This script will generate an external current file for insilico with currents for different groups of neurons, where for we specify gaussian distribution of the dc current being given to the population of neurons (like in 50 neurons, n10 gets i=4, (n9 and n11) get i=3, etc.)
...
start time	10 ms	10 ms	10 ms
end time	50 ms	50 ms	50 ms
 
python /address/script_name.py /address/outfilename.ecf 2 2,1 10,20 5,3,4,10,50*5,6,7,10,50*50,8,11,10,50  
...
arg 5	max current value, width, mean, start time and end time for each gaussian (separated by commas) and for all gaussian separated by * (eg 5,3,4,10,50*5,6,7,10*50,8,11,10,50)
...
for t in range(tN):
	...
	for nG in range(nGa[t]):
		h = float(allD[t][nG][0])
		sigma = int(allD[t][nG][1])
		mu = int(allD[t][nG][2])
		I_val += getIval(nNa[t], h, sigma, mu)
...
time_list = np.ndarray.tolist(np.arange(start_time_all, end_time_all + step, step))
